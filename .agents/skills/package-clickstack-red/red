#!/usr/bin/env bun
// ClickStack launcher — a ClickHouse observability stack on Vultr.
//
...
//
// COLORS_PAR_PROFILE is the exception — clickstack refuses to run when it is
...
// can only point clickstack at another project's state.
//
...
// own: validation, the graph and the steps all live in the `package-clickstack-red`
...
//
// "package-clickstack-red" is null before this repository has been pushed and
...
// rewrites null to "github:getcolors/clickstack#<sha>" in place.
//
...
const PINS = {
  "package-clickstack-red": "github:getcolors/clickstack#68887b5055a31c11be9a808b6a1fe0974755bb13",
...
// add-deps. CLICKSTACK_LIB_ROOT overrides everything with a working tree: point
// it at the `red/` directory of a clickstack checkout whose dependencies are
 ...
 *
 * The payload's canonical home is skills/package-clickstack-red/ inside the
 * clickstack repository, whose working tree is the point: resolving a pinned
...
function checkoutRedDir() {
  ...
  return manifest?.name === "package-clickstack-red-dev" ? candidate : null;
}
...
function installToCache() {
  ...
  const root = join(home, "package-clickstack-red");
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "package-clickstack-red-cache", private: true, dependencies: PINS }, null, 2)}\n`,
  );
...
let clickstack;
try {
  clickstack = await import("package-clickstack-red");
} catch (err) {
  ...
  const libRoot = process.env.CLICKSTACK_LIB_ROOT;
  ...
  if (libRoot) {
    ...
    clickstack = await importWorkingTree(overrideDir, "CLICKSTACK_LIB_ROOT");
    ...
    clickstack = await importWorkingTree(redDir, "checkout");
  } else if (!PINS["package-clickstack-red"]) {
    console.error(
      "red: this launcher carries no clickstack pin.\n" +
        "It has not been stamped by `bb pin`, which cannot run until the " +
        "clickstack repository has been pushed.\n" +
    ...
    // not to a path.
    clickstack = await import(Bun.resolveSync("package-clickstack-red", dir));
  }
}

export const run = clickstack.run;
if (import.meta.main) await clickstack.exec();
